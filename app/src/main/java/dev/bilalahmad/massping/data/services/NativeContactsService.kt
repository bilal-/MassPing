...
class NativeContactsService(private val context: Context) {
    ...
    suspend fun fetchContactGroups(): Result<List<ContactGroup>> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val groupCursor = context.contentResolver.query(
                    ...
                    "${ContactsContract.Groups.DELETED} = 0 AND ${ContactsContract.Groups.TITLE} IS NOT NULL",
                    null,
                ...
                groupCursor?.use { cursor ->
                    while (cursor.moveToNext()) {
                        ...
                        val contactIds = getContactsInGroup(groupId)
                ...
                Log.d(TAG, "Fetched ${groups.size} contact groups")
    ...
    private fun getContactsInGroup(groupId: String): List<String> {
